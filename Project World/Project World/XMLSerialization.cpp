...
namespace xmls
{
	...
	*/
	...
	/**
	Assign a value to the xfloat member
	@return	void
	*/
	...
	*/
	...
	@return void
	*/
	void Serializable::Register(std::string MemberName, MemberBase *Member, std::string DefaultValue)
	{
	...
	@return void
	*/
	void Serializable::Register(std::string MemberName, Serializable *Member)
	{
	...
	@return void
	*/
	void Serializable::Register(std::string CollectionName, CollectionBase *SubclassCollection)
	{
	...
	*/
	void Serializable::Serialize(tinyxml2::XMLDocument *classDoc, tinyxml2::XMLElement *rootNode)
	{
		...
		{
			std::string *field = (*it_member)->getField();
	...
	*/
	void Serializable::Deserialize(Serializable *classItem, tinyxml2::XMLDocument *classDoc, tinyxml2::XMLElement *rootNode)
	{
		...
		{
			std::string fieldName = memberNode->Attribute("Name");
		...
		{
			std::string className = classNode->Attribute("Name");
		...
		{
			std::string collectionName = collectionNode->Attribute("Name");
	...
	*/
	bool Serializable::fromXML(std::string XMLSource, Serializable *classObject)
	{
		bool result=false;
		tinyxml2::XMLDocument *classDoc = new tinyxml2::XMLDocument();
		tinyxml2::XMLElement *rootNode;
	...
	@return class name
	*/
	std::string Serializable::IdentifyClass(std::string XMLSource)
	{
	...
	*/
	std::string Serializable::IdentifyClassVersion(std::string XMLSource)
	{
		tinyxml2::XMLDocument doc;
		doc.Parse(XMLSource.c_str(), (size_t)XMLSource.length());
		tinyxml2::XMLElement* rootNode;
	...
	*/
	std::string Serializable::strReplaceAll(std::string source, const std::string searchFor, const std::string replaceWith)
	{
		if(searchFor.empty())
			return source;
		size_t start_pos = 0;
	...
	*/
	void Serializable::Replace(std::string searchFor, std::string replaceWith, bool recursive)
	{
		for (FieldMappingIterator it = m_FieldMappings.begin() ; it != m_FieldMappings.end(); ++it)
